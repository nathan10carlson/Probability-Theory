...
if __name__ == "__main__":
    ...
    thta = np.array([0.1, 0.1, 0.1, 0.1, 1, 1, 1, 1])

    # the output of the solver is the solution on a fine and coarse grid
    x, u, x_16, u_16 = solve_Poisson(thta)
    # x = fine-grained grid
    # u = solution values on fine-grained grid
    # x_16 = 16 measurement locations
    # u_16 = exact solution values at 16 measurement locations

    # demo of what the function computes
    plt.figure()
    plt.plot(x, u)
    plt.xlabel("x")
    plt.ylabel("u(x)")
    plt.plot(x_16, u_16, "xr")
    plt.legend(["solution", "solution at measurement locations"])
    plt.show()
